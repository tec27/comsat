...
Replay.prototype = {
  ...
  _mergeData: function(data, callback) {
    if(!data.header && !data.detail && !data.attributes && !data.messages) { // TODO: add the rest of the data files to this list
    ...
    if(data.details) {
      ...
      var unixDate = data.details.datetime.subtract(new BigInteger('116444735995904000')).divide(BigInteger.valueOf(10).pow(7)).intValue(); // converts from Windows nano time to unix time (so we don't have to deal with bigints outside of this module)
      var timezoneOffsetSec = data.details.timezoneOffset.divide(BigInteger.valueOf(10).pow(7)).intValue();
      this.info.date = { value: unixDate, timezoneOffset: timezoneOffsetSec };
      this.info.numPlayers = data.details.players.length;
    }

    if(data.attributes) {
      var attrNames = attributesParser.attributeNames;
      if(data.attributes[attrNames.PlayerType]) {
        var attrib = data.attributes[attrNames.PlayerType];
        ...
        for(var i = 0; i < attrib.length; i++) {
          ...
          if(type != 'Open') {
            ...
            if(data.attributes[attrNames.PlayerRace] && data.attributes[attrNames.PlayerRace][i])
              this.players[i].race = attributesParser.playerRace(data.attributes[attrNames.PlayerRace][i]);

            if(data.attributes[attrNames.PlayerColor] && data.attributes[attrNames.PlayerColor][i])
              this.players[i].color.name = attributesParser.playerColor(data.attributes[attrNames.PlayerColor][i]);

            if(data.attributes[attrNames.Difficulty] && data.attributes[attrNames.Difficulty][i])
              this.players[i].difficulty = attributesParser.difficulty(data.attributes[attrNames.Difficulty][i]);
          }
        }
      }
      var teamAttrib = null;
      if(data.attributes[attrNames.Format]) {
        this.info.format = attributesParser.format(data.attributes[attrNames.Format][15]);
        if(this.info.format !== 'Custom') teamAttrib = 'Teams' + this.info.format;
      }
      if(data.attributes[attrNames.GameSpeed]) {
        this.info.speed = attributesParser.gameSpeed(data.attributes[attrNames.GameSpeed][15]);
      }
      if(data.attributes[attrNames.GameType]) {
        this.info.gameType = attributesParser.gameType(data.attributes[attrNames.GameType][15]);
      }
      if(teamAttrib && data.attributes[attrNames[teamAttrib]]) {
        var attrib = data.attributes[attrNames[teamAttrib]];
    ...
    (function(replay) {
      mpyq.extractReplay(replay.filename, deleteFile, function(err,extractDir) {
        ...
        fs.readFile(headerFile, function onHeaderRead(err, data) { 
          headerParser.parse(err, data, function onHeaderParsed(err, header) {
            if(err) {
              callback(err);
              return;
            }
            replay._processHeader({ header: header });
            var version = replay.info.version;
